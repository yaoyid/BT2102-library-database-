...
def info(accessionid, memberid):
    try:
        local_session = session(bind = engine)
        duedate = datetime.datetime.now().date() + timedelta(14)
...
def returning(accessionid, datereturn):
    ...
    a = select(*BookLoan.__table__.columns).where(BookLoan.accessionNo == accessionid)
    a = local_session.execute(a).first()
    dat = datetime.datetime.strptime(datereturn, '%Y-%m-%d').date()
    if a:
...
def reserve(accessionid, memberid, reservedate):
    ...
    if notborrowed(accessionid):
        return (False, "book available now")
    if acquire_fine(memberid) > 0:
...
def deletereserve(accessionid, memberid, canceldate):
    ##if not notborrowed(accessionid) and memberReserved(memberid, accessionid):
    local_session = session(bind = engine)
    ...
    return True
        ...
        elif notborrowed(accessionid):
            return (False, "book available now")
    """

def member_getName(memberid):
...
def is_member(memberid):
    con = create_connection()
    cur = con.cursor()
    comment = "SELECT * FROM Membership WHERE memberID = %s"
    cur.execute(comment, memberid)
    result = cur.fetchall()
    if len(result) == 0:
        return False
    return True

def book_exist(accessionNo):
    con = create_connection()
    cur = con.cursor()
    comment = "SELECT * FROM Book WHERE accessionNo = %s"
    cur.execute(comment, accessionNo)
    result = cur.fetchall()
    if len(result) == 0:
        return False
    return True

def book_getTitle(accesionNo):
    con = create_connection()
    cursor = con.cursor()
    query = "SELECT title From Book WHERE accessionNo =  %s"
    cursor.execute(query,accesionNo)
    result = cursor.fetchall()
